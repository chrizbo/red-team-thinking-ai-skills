...
Run this as dialogue, not memos. Keep each persona's individual statement under about 80 words — real people speaking in turns, not essays stacked together.
...
## Step 5: Mid-exercise checkpoint

Before synthesizing, stop and ask the user one direct question — e.g., which line of attack they want pushed harder, or whether there's context that would change the panel's read. If they redirect or add context, let the personas react to it in the next step without going soft — new context should sharpen the critique or shift its target, not defuse it into diplomacy.

## Step 6: Synthesize

Collapse the three passes (and any reaction to the user's Step 5 input) into a ranked list of concrete failure points, each with a one-line suggested mitigation. Rank by how damaging the failure would be, not by which persona raised it.
